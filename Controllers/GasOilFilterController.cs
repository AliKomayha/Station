...
{
    public class GasOilFilterController : Controller
    {
        private readonly string _connectionString;

        public GasOilFilterController(string connectionString)
        {
        ...
        public IActionResult Index()
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("SELECT Id, Type, Quantity, Price, Date, UsersID FROM GasOilFilters", connection);
            ...
            if (ModelState.IsValid)
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    ...
                    var command = new SqlCommand(
                        "INSERT INTO GasOilFilters (Type, Quantity, Price, Date, UsersID) VALUES (@Type, @Quantity, @Price, @Date, @UsersID)",
                        connection);
                    ...
                    command.Parameters.AddWithValue("@UsersID", gasOilFilter.UserId);
        ...
        public IActionResult Edit(GasOilFilter gasOilFilter)
        {
            if (ModelState.IsValid)
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    connection.Open();
                    var command = new SqlCommand(
                        "UPDATE GasOilFilters SET Type=@Type, Quantity=@Quantity, Price=@Price, Date=@Date, UsersID=@UsersID WHERE Id=@Id",
                        connection);
                    command.Parameters.AddWithValue("@Type", gasOilFilter.Type);
                    command.Parameters.AddWithValue("@Quantity", gasOilFilter.Quantity);
                    command.Parameters.AddWithValue("@Price", gasOilFilter.Price);
        ...
        public IActionResult DeleteConfirmed(int id)
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("DELETE FROM GasOilFilters WHERE Id=@Id", connection);
        ...
        private GasOilFilter? GetGasOilFilterById(int id)
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("SELECT Id, Type, Quantity, Price, Date, UsersID FROM GasOilFilters WHERE Id=@Id", connection);
                command.Parameters.AddWithValue("@Id", id);
                var reader = command.ExecuteReader();
                if (reader.Read())
                {
